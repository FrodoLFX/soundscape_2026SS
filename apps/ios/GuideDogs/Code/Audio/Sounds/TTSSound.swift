...
/// Object that encapsulates the rendering of text to speech using Apple's AVSpeechSynthesizer engine.
/// This sound object asynchronously generates audio buffers for the speech.
class TTSSound: Sound {
    ...
    // NEW: When using the device speaker, prefer non-spatialized speech for clarity
    private static var isBuiltInSpeakerOutput: Bool {
    ...
    init(_ text: String, at: CLLocation) {
        self.text = text
        self.type = TTSSound.isBuiltInSpeakerOutput ? .standard : .localized(at, .ring) // NEW
    }
    ...
    init(_ text: String, direction: CLLocationDirection) {
        self.text = text
        self.type = TTSSound.isBuiltInSpeakerOutput ? .standard : .relative(direction, .ring) // NEW
    }
    ...
    init(_ text: String, compass: CLLocationDirection) {
        self.text = text
        type = TTSSound.isBuiltInSpeakerOutput ? .standard : .compass(compass, .ring) // NEW
    }
